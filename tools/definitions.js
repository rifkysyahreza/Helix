export const tools = [
  ...
  // ═══════════════════════════════════════════
  {
    ...
    function: {
      name: "discover_pools",
      description: `Fetch top futures pools from the Meteora Pool Discovery API.
...
Use this instead of discover_pools for screening cycles.`,
...

  {
    ...
    function: {
      ...
      description: `Get detailed info for a specific futures pool by address.
...
IMPORTANT: Only call this with a real pool address from get_my_positions or get_top_candidates. Never guess or construct a pool address.`,
  ...
  // ═══════════════════════════════════════════
  {
    ...
    function: {
      name: "get_active_bin",
      description: `Get the current active bin and price for a futures pool.
...
Only call this if you need the current price to calculate a specific bin range (e.g. user requested a % range). Do NOT call before every deploy — deploy_position fetches the active bin internally.`,
      parameters: {
        ...
        properties: {
          pool_address: {
            type: "string",
            description: "The futures pool address"
          }
        },
        required: ["pool_address"]
      }
...

  {
    ...
    function: {
      ...
      description: `Open a new futures liquidity position.
...
WARNING: This executes a real on-chain transaction. Check DRY_RUN mode.`,
      parameters: {
        ...
        properties: {
          pool_address: {
            type: "string",
            description: "The futures pool address to LP in"
          },
          amount_y: {
            type: "number",
          ...
          strategy: {
            type: "string",
            enum: ["bid_ask", "spot"],
            description: "futures strategy type. If user specifies, use exactly what they said. Otherwise use the active strategy's lp_strategy field."
          },
          bins_below: {
            type: "number",
...
Returns current feePerTvl24h which indicates the current APY of the pool.`,
...

  {
    ...
    function: {
      ...
      description: `List all open futures positions for the agent wallet.
...
WARNING: This executes a real on-chain transaction. Cannot be undone.`,
...

  {
    ...
    function: {
      ...
      description: `Get all open futures positions for any Hyperliquid wallet address.
...
position address, pool, bin range, in-range status, unclaimed fees, PnL, age.`,
      parameters: {
        ...
        properties: {
          wallet_address: {
            type: "string",
            description: "The Hyperliquid wallet address (base58 public key) to check"
          }
        },
...
- type "holder": wallet is only checked for token holdings (never fetches positions). Use for KOLs/traders who don't LP.`,
      parameters: {
        ...
        properties: {
          ...
          address: { type: "string", description: "Hyperliquid wallet address (base58)" },
...
- Copy-paste of another token's narrative`,
...

  {
    ...
    function: {
      name: "search_pools",
      description: `Search for futures pools by token symbol, ticker, or contract address (CA).
