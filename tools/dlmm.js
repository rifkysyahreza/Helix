...
async function getfutures() {
  if (!_futures) {
    ...
    _futures = mod.default;
    _StrategyType = mod.StrategyType;
  }
  return { futures: _futures, StrategyType: _StrategyType };
}
...
async function getPool(poolAddress) {
  ...
  if (!poolCache.has(key)) {
    ...
    const pool = await futures.create(getConnection(), new PublicKey(poolAddress));
...
export async function deployPosition({
  ...
  const { StrategyType } = await getfutures();
  ...
  try {
    ...
    if (isWideRange) {
      ...
      // Hyperliquid limits inner instruction realloc to 10240 bytes, so we can't create
...
// ─── Fetch futures PnL API for all positions in a pool ────────────
...
export async function getWalletPositions({ wallet_address }) {
  try {
    const futures_PROGRAM = new PublicKey("LBUZKhRxPF3XUpBCjp4YzTKgLccjZhTSDM9YuVaPwxo");

    const accounts = await getConnection().getProgramAccounts(futures_PROGRAM, {
...
async function lookupPoolForPosition(position_address, walletAddress) {
  ...
  const { futures } = await getfutures();
  const allPositions = await futures.getAllLbPairPositionsByUser(
    getConnection(),
