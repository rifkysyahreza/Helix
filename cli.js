#!/usr/bin/env node
/**
 * helix — Hyperliquid futures LP Agent CLI
...
// ─── Load .env from ~/.helix/ if present ──────────────────────
...
// ─── SKILL.md generation ──────────────────────────────────────────
const SKILL_MD = `# helix — Hyperliquid futures LP Agent CLI
...
### helix token-info --query <mint_or_symbol>
...
### helix token-holders --mint <addr> [--limit 20]
...
switch (subcommand) {
  ...
  case "pnl": {
    ...
    if (!positionAddress) die("Usage: helix pnl <position_address>");
  ...
  case "token-info": {
    ...
    if (!query) die("Usage: helix token-info --query <mint_or_symbol>");
  ...
  case "token-holders": {
    ...
    if (!mint) die("Usage: helix token-holders --mint <addr>");
  ...
  case "token-narrative": {
    ...
    if (!mint) die("Usage: helix token-narrative --mint <addr>");
  ...
  // ── pool-detail ───────────────────────────────────────────────
  case "pool-detail": {
    if (!flags.pool) die("Usage: helix pool-detail --pool <addr> [--timeframe 5m]");
  ...
  case "search-pools": {
    ...
    if (!query) die("Usage: helix search-pools --query <name_or_symbol>");
  ...
  // ── active-bin ────────────────────────────────────────────────
  case "active-bin": {
    if (!flags.pool) die("Usage: helix active-bin --pool <addr>");
    const { getActiveBin } = await import("./tools/dlmm.js");
  ...
  case "wallet-positions": {
    ...
    if (!wallet) die("Usage: helix wallet-positions --wallet <addr>");
  ...
  // ── deploy ───────────────────────────────────────────────────────
  case "deploy": {
    if (!flags.pool) die("Usage: helix deploy --pool <addr> --amount <sol>");
  ...
  // ── claim ────────────────────────────────────────────────────────
  case "claim": {
    if (!flags.position) die("Usage: helix claim --position <addr>");
    const { executeTool } = await import("./tools/executor.js");
  ...
  // ── close ────────────────────────────────────────────────────────
  case "close": {
    if (!flags.position) die("Usage: helix close --position <addr>");
    const { executeTool } = await import("./tools/executor.js");
  ...
  // ── swap ─────────────────────────────────────────────────────────
  case "swap": {
    if (!flags.from || !flags.to || !flags.amount) die("Usage: helix swap --from <mint> --to <mint> --amount <n>");
    const { executeTool } = await import("./tools/executor.js");
  ...
  case "config": {
    if (sub2 === "get" || !sub2) {
      ...
      if (!key || rawVal === undefined) die("Usage: helix config set <key> <value>");
      let value = rawVal;
  ...
  // ── study ────────────────────────────────────────────────────────
  case "study": {
    if (!flags.pool) die("Usage: helix study --pool <addr> [--limit 4]");
  ...
  case "start": {
    ...
    process.stderr.write("[helix] Starting autonomous agent...\n");
    startCronJobs();
    break;
  }
  ...
  case "lessons": {
    if (sub2 === "add") {
      ...
      if (!text) die("Usage: helix lessons add <text>");
  ...
  // ── pool-memory ──────────────────────────────────────────────────
  case "pool-memory": {
    if (!flags.pool) die("Usage: helix pool-memory --pool <addr>");
  ...
  case "blacklist": {
    if (sub2 === "add") {
      if (!flags.mint) die("Usage: helix blacklist add --mint <addr> --reason <text>");
  ...
  case "withdraw-liquidity": {
    if (!flags.position) die("Usage: helix withdraw-liquidity --position <addr> --pool <addr> [--bps 10000]");
  ...
  // ── add-liquidity ──────────────────────────────────────────────
  case "add-liquidity": {
    if (!flags.position) die("Usage: helix add-liquidity --position <addr> --pool <addr> [--amount-x <n>] [--amount-y <n>]");
    if (!flags.pool) die("--pool is required");
