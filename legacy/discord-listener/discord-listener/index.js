/**
 * helix Discord listener — selfbot
 * Watches LP Army channels for Hyperliquid addresses and runs pre-check pipeline.
...
// Load .env from parent directory (helix root)
...
// Hyperliquid address regex: base58, 32-44 chars
...
const FALSE_POSITIVE_SKIP = new Set([
  "hyperliquid", "meteora", "jupiter", "raydium", "orca",
]);

function isLikelyHyperliquidAddress(str) {
...
client.on("ready", () => {
  console.log(`\n[helix discord-listener] Connected as ${client.user?.tag}`);
...
client.on("messageCreate", async (message) => {
  ...
  const unique = [...new Set(matches)].filter(isLikelyHyperliquidAddress);
