...
export function evaluatePositionManagement({ trade = null, livePosition = null, analysis = null } = {}) {
  ...
  const profitProtection = evaluateProfitProtection(trade.tradeId, { livePosition, analysis });
  ...
  } else if (profitProtection.action === "reduce") {
    action = "reduce";
    reducePct = 25;
    reason = profitProtection.reason;
  } else if (profitProtection.action === "raise_stop") {
    action = "protect";
    reason = profitProtection.reason;
